...
def depthFirstSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        if node.position not in closed:
            ...
            for child_node in problem.getSuccessors(node.position):
                fringe.push(Node(child_node[0], node.path + [child_node[1]]))
    return None
...
def uniformCostSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        if node.position not in closed:
            ...
            for child_node in problem.getSuccessors(node.position):
                fringe.push(Node(child_node[0], node.path + [child_node[1]], \
                child_node[2] + node.priority), child_node[2] + node.priority)
    return None
...
def heuristic(state, problem):
    return util.manhattanDistance(state)
